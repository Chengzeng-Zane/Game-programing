...
public class UIManager : MonoBehaviour
{
    ...
    public void GoToPage(int pageIndex)
    {
        ...
        {
            Debug.LogWarning("UIManager could not open page index " + pageIndex + ".");
        }
    }
    ...
    public void GoToPageByName(string pageName)
    {
        ...
        if (pageIndex < 0)
        {
            Debug.LogWarning("UIManager could not find a page named " + pageName + ".");
            return;
        }
        GoToPage(pageIndex);
    }
    ...
    /// <summary>
    /// Description:
    /// Hides level select buttons for scenes that are not included in the build settings.
    /// This keeps the menu connected only to playable levels that actually exist.
    /// Inputs:
    /// none
    /// Returns:
    /// void (no return)
    /// </summary>
    private void HideUnavailableLevelButtons()
    {
        HideLevelButtonIfSceneMissing("LevelOneButton", "Level1");
        HideLevelButtonIfSceneMissing("LevelTwoButton", "Level2");
        HideLevelButtonIfSceneMissing("LevelThreeButton", "Level3");
    }

    /// <summary>
    /// Description:
    /// Finds a named button and hides it if its scene is missing from the build settings.
    /// Inputs:
    /// string buttonName, string sceneName
    /// Returns:
    /// void (no return)
    /// </summary>
    private void HideLevelButtonIfSceneMissing(string buttonName, string sceneName)
    {
    ...
    /// <summary>
    /// Description:
    /// Checks whether a scene name is listed in the build settings.
    /// Inputs:
    /// string sceneName
    /// Returns:
    /// bool
    ...
    /// <summary>
    /// Description:
    /// Finds a scene object by name, including objects under inactive UI pages.
    /// Inputs:
    /// string objectName
    /// Returns:
    /// GameObject
