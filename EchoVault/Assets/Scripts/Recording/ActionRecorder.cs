...
namespace EchoVault
{
    public class ActionRecorder : MonoBehaviour
    {
        ...
        public void PlayEcho()
        {
            if (!HasRecording)
            {
                EchoVaultGameManager.Instance?.UpdateStatus("Record a movement first with Q, then press E to replay it.");
                return;
            }
            ...
            EchoVaultGameManager.Instance?.AudioService?.PlayTap();
            EchoVaultGameManager.Instance?.UpdateStatus("Echo replay started. Use it to hold switches or time hazards.");
        }
        ...
        private void StartRecording()
        {
            ...
            IsRecording = true;
            EchoVaultGameManager.Instance?.AudioService?.PlayTap();
            EchoVaultGameManager.Instance?.UpdateStatus("Recording. Move, jump, or stand on a plate, then press Q to stop.");
        }
        ...
        private void StopRecording()
        {
            IsRecording = false;
            EchoVaultGameManager.Instance?.AudioService?.PlayTap();
            EchoVaultGameManager.Instance?.UpdateStatus($"Recording saved: {frames.Count} frame(s). Press E to replay your echo.");
        }
    }
}
